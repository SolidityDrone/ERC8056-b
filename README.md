...
| 3 | Base `effectiveAt()` | the single pending effective timestamp | earliest pending `effectiveAt` across classes; `0` when nothing is pending on any class (vanilla sentinel idiom restored) |
