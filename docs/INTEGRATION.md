...
| Member | Notes |
|--------|-------|
| `unwrap(amount, start, target)` | burn **both** legs, receive exactly `amount` — anytime. **Returns** the raw amount released (== `amount`) |
| `unwrapYield(amount, start, target)` | burn `amount` yield leg for `amount * coupon`. **Returns** the raw amount released |
| `unwrapCapital(amount, start, target)` | burn `amount` capital leg for `amount * (1 - coupon)`. **Returns** the raw amount released |
